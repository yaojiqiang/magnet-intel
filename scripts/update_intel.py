...
def build_prompt(existing):
    ...
    return (
        ...
        "⚠️ 价格来源优先级：①我的钢铁网（首选，行情、现货、月度均价、价格指数优先以其为准）；②若我的钢铁网无法获取某品类或某月数据，再到亚洲金属网查找；③亚洲金属网也查不到时再到百川盈孚查找。使用替代来源时必须如实标注实际平台名称与报价日期，严禁把替代来源虚标为我的钢铁网。
"
        "⚠️ 时效性：我的钢铁网当日行情快讯通常在上午 11:00 后发布，每日运行当日快讯一般已发布，应优先取用当日（当天日期）报价；若个别品类当日快讯未披露（如周末、节假日当日无行情），则 date 填最近可获取的实报价日期（如取到 8/12 快讯就填 2026-08-12），不得虚标为运行当天日期，并在 updateNote 中注明『当日无该品类最新快讯，沿用最近报价』。
"
...
def build_activities_prompt(existing):
    ...
    return (
        ...
        "  ⚠️ 维度归类须与内容实质严格匹配：工艺/技术类内容（晶界扩散、无重稀土、磁能积提升、研发工艺突破等）一律归 tech，不得错归为 digital；仅智能工厂、AI质检、自动化产线、数字化升级等智能制造/数字化内容才归 digital。
"
        "  market  → 市场\n  tech    → 工艺技术\n  supply  → 供应链\n  digital → 数字化\n\n"
        "【时间范围】" + window + "\n\n"
        "【输出要求】\n"
        ...
        "每次更新尽量补充最新发现的动态（业绩预告/回购/调研/专利/扩产/合作等），使竞社动态保持充足数量；description 须充实具体（含关键数字、金额、比例、背景），不得只写一句话；来源可重点参考各公司及行业协会微信公众号（source 标注公众号名，sourceUrl 填原文链接）。
"
        "仅返回 JSON 对象，不要任何解释文字或 Markdown 围栏。"
    )
